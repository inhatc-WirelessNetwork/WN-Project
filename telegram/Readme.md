...
* 작업 스레드 생성 과정을 코드를 통해 살펴보겠습니다.
  ```
    stop_event = threading.Event()
    stop_event.clear()
  ```
    + 특정 이벤트를 받는 객체를 생성 후, 특정 이벤트 발생 전까지 대기
  ```
    loop = asyncio.get_event_loop()
    loop.create_task(start_scoliosis(update))
  ```
  
  + ```asyncio.get_event_loop()``` : 현재 쓰레드의 이벤트 루프를 가져옴
  + ```loop.create_task(Coroutine function())``` : ***Coroutine function*** 이란  함수의 실행을 중지하고 나중에 필요한
         시점에 다시 시작할 수 있는 특별한 종류의 함수입니다.
       ```create_task()``` 함수는 코루틴 함수를 파라미터로 전달받아 태스크를 생성해 비동기적으로 실행되게 합니다.
    <br>
  ```
    stop_event.set()
  ```
    + 이벤트 객체의 상태를 설정하는 메서드
    + 현재 동작 중인 태스크들이 중지
